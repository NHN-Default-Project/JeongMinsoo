package com.nhnacademy.simpleanimationstarter;
...
public class SimpleAnimationStarter extends Application {
    ...
    public void drawFrame(GraphicsContext g, int frameNumber, double elapsedSeconds, int width, int height) {
        ...
        int boxSize = 100;
        int range = width - boxSize;
        int boxCount = 6;

        //선 그리기
        for (int i = 1; i < boxCount; i++) {
            g.strokeLine(0, (double) boxSize * i, width, (double) boxSize * i);
        }

        Color[] colorArray = {Color.RED, Color.GREEN, Color.BLUE, Color.CYAN, Color.MAGENTA, Color.YELLOW};

        drawRect(g, colorArray, frameNumber, boxCount, range, boxSize);
    }

    public void drawRect(GraphicsContext g, Color[] colorArray, int frameNumber, int boxCount, int range, int size) {
        for (int i = 0; i < boxCount; i++) {
            if (i < boxCount / 2) {
                int cyclicFrameNumber = frameNumber * (i + 5) % range;

                g.setFill(colorArray[i]);
                g.fillRect(cyclicFrameNumber, (double) size * i, size, size);
            } else {
                int oscilationFrameNumber = frameNumber * (i + 5) % (2 * range);
                if (oscilationFrameNumber > range) {
                    oscilationFrameNumber = (2 * range) - oscilationFrameNumber;
                }

                g.setFill(colorArray[i]);
                g.fillRect(oscilationFrameNumber, (double) size * i, size, size);
            }
        }
    }
